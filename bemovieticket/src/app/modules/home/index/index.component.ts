...
export class IndexComponent implements OnInit {
  ...
  constructor(private homeService: HomeService,configRating: NgbRatingConfig){
    //Cài đặt rating
    configRating.max = 5;
  ...
  //lấy list phim
  getListFilms() {
    this.homeService.getListFilms().subscribe(res => {
      this.listFilms = res
    })
  }
  ...
  getListSystemTheaters() {
    this.homeService.getListSystemTheaters().subscribe(res => {
  ...
  showTheaters(systemTheaterID: string, type: number = 0) {
    if (type == 0) {
      ...
      this.homeService.getListTheaters(systemTheaterID).subscribe(res => {
        this.listTheaters = res
      })
    } else {
      if (!this.isInitShowTime) {
        ...
        // this.isInitShowTime = false        //nghiên cứu chỗ này
        this.systemTheaterId = systemTheaterID;
        this.homeService.getListTheaters(systemTheaterID).subscribe(res => {
          this.listTheaters = res;
  ...
  showFilmsOfTheater(theaterID: string, type: number = 0) {
    if (type == 0) {
      ...
      if (!this.isInitShowTime) {
        ...
        this.homeService.getInfoShowtimeTheater(this.systemTheaterId).subscribe(res => {
          let listTheaters = res[0].lstCumRap;
          let hasShowTime = listTheaters.find(x => x.maCumRap == theaterID)
          if (hasShowTime) {
            this.listFilmsOfTheater = hasShowTime.danhSachPhim
